...
public class RouteFixBundle implements ConfiguredBundle<GraphHopperBundleConfiguration> {
    ...
    public void run(GraphHopperBundleConfiguration configuration, Environment environment) {
        ...
        if (configuration.getGraphHopperConfiguration().has("gtfs.file")) {
            throw new UnsupportedOperationException("PT routing not supported by osmroutefix");
        } else {
    ...
    private void runRegularGraphHopper(CmdArgs configuration, Environment environment) {
        final RouteFixManaged graphHopperManaged = new RouteFixManaged(configuration, environment.getObjectMapper());
        environment.lifecycle().manage(graphHopperManaged);
        environment.jersey().register(new AbstractBinder() {
            @Override
            protected void configure() {
                bind(configuration).to(CmdArgs.class);
                bind(graphHopperManaged).to(RouteFixManaged.class);
