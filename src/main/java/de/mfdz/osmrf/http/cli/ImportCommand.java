...
package de.mfdz.osmrf.http.cli;

import com.graphhopper.http.GraphHopperServerConfiguration;
import de.mfdz.osmrf.graphhopper.RouteFixManaged;
...
public class ImportCommand extends ConfiguredCommand<GraphHopperServerConfiguration> {
    ...
    protected void run(Bootstrap<GraphHopperServerConfiguration> bootstrap, Namespace namespace, GraphHopperServerConfiguration configuration) throws Exception {
        if (configuration.getGraphHopperConfiguration().has("gtfs.file")) {
            throw new IllegalArgumentException("GTFS is not supported");
        } else {
            final RouteFixManaged graphHopper = new RouteFixManaged(configuration.getGraphHopperConfiguration(), bootstrap.getObjectMapper());
            graphHopper.start();
            graphHopper.stop();
        }
