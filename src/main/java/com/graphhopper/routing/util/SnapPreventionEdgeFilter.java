...
public class SnapPreventionEdgeFilter implements EdgeFilter {
    ...
    private final BooleanEncodedValue rclEnc;
    private final EdgeFilter filter;
    private boolean avoidMotorway = false, avoidTrunk, onlyLinks;
    ...
    public SnapPreventionEdgeFilter(EdgeFilter filter, EnumEncodedValue<RoadClass> rcEnc,
            EnumEncodedValue<RoadEnvironment> reEnc, BooleanEncodedValue rclEnc,
        ...
        for (String roadClassOrRoadEnv : snapPreventions) {
            if ("link".equals(roadClassOrRoadEnv)) {
                onlyLinks = true;
                continue;
            } else if ("motorway".equals(roadClassOrRoadEnv)) {
                avoidMotorway = true;
                continue;
    ...
    public boolean accept(EdgeIteratorState edgeState) {
        return filter.accept(edgeState)
                ...
                && (!onlyLinks || edgeState.get(rclEnc));
    }
}
